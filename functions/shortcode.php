...
function AnyContact_ShortCode($args = false) {
	...
	// fix the wordpress shit
	array_walk_recursive($_POST,'stripslashes_fix');
	
  if(!$args || $args == "" || !array_key_exists('id',$args) || !is_numeric($args['id'])) {
    AnyContact_Frontend_Error(__('Ihr Formular wurde falsch konfiguriert. Bitte überprüfen Sie Einstellungen und den Shortcode.','anycontact'));
    return;
  }
  AnyContact_Frontend_HTML($args['id']);
}

function AnyContact_Frontend_CheckPost() {
	
	if(!array_key_exists('pac_id',$_POST)) {
		AnyContact_Frontend_Error(__('Die übermittelte ID enthält leider kein Formular.','anycontact'));
		return;
	}
...
function AnyContact_Frontend_HTML($id) {
  ...
  $post = get_post($id,'ARRAY_A');
	...
	if(!is_array($post) || sizeof($post) == 0) {
    echo __('Die übermittelte ID enthält leider kein Formular.','anycontact');
    return;
  }
	
  $form_args = json_decode($post['post_content'],true);
  ?>
  <!-- START WP-Plugin AnyContact http://any-contact.com -->
	<div id="pac-form-css">
    <?
      ...
      if($check_post === true) {
        ...
        $send_message = wp_mail($to,$subject,$message,$headers);
				...
				if($send_message === true) {
					AnyContact_Frontend_SuccessBox($form_args['text']['success']);
					$form_load = false;
				} else {
					AnyContact_Frontend_ErrorBox($form_args['text']['error_code']);
				}
      } else {
				$error = $check_post;
				AnyContact_Frontend_ErrorBox($form_args['text']['error_form']);
      }
    }
		
		if($form_load) {
			?>
			<form id="pac-form-css" method="post" action="">
				...
				foreach($form_args['field'] AS $key => $args) {
					...
					<p>
						...
						if(in_array($name,$error)) {
							AnyContact_Frontend_Error($form_args['text']['error_'.$args['type']]);
					...
					<p class="pac">
						...
						switch($args['type']) {
							...
							case 'captcha':
								?>
								<img src="<?= ANYCONTACT_HTTP;?>captcha.php" title="Captcha" />
								<input <?= $class;?> id="<?= $name;?>" name="<?= $name;?>" type="text" value="" autocomplete="off" required />
								<?
								break;
								
							case 'character':
								?>
								<input <?= $class;?> id="<?= $name;?>" name="<?= $name;?>" type="text" value="<?= $_POST[$name];?>" <?= ($args['required']=='true')?'required':false;?>/>
								<?
								break;
								
							case 'checkbox':
								?>
								<input <?= $class;?> id="<?= $name;?>" name="<?= $name;?>" type="checkbox" value="checked" <?= $_POST[$name] == 'checked'?'checked':false;?> <?= ($args['required']=='true')?'required':false;?>/>
								<?
								if(array_key_exists('link_title',$args) && array_key_exists('link_url',$args)) {
									?>
									<span><a href="<?= $args['link_url'];?>" target="_blank"><?= $args['link_title'];?></a></span>
									<?
								}
							...
							case 'date':
								?>
								<input <?= $class;?> id="<?= $name;?>" name="<?= $name;?>" type="date" value="<?= $_POST[$name];?>" <?= ($args['required']=='true')?'required':false;?>/>
								<?
								break;
								
							case 'mail':
								?>
								<input <?= $class;?> id="<?= $name;?>" name="<?= $name;?>" type="email" value="<?= $_POST[$name];?>" <?= ($args['required']=='true')?'required':false;?>/>
								<?
								break;
								
							case 'number':
								?>
								<input <?= $class;?> id="<?= $name;?>" name="<?= $name;?>" type="number" value="<?= $_POST[$name];?>" <?= ($args['required']=='true')?'required':false;?>/>
								<?
								break;
							...
							case 'select':
								...
								?>
								<select <?= $class;?> id="<?= $name;?>" name="<?= $name;?>" <?= ($args['required']=='true')?'required':false;?>>
								<?
								foreach($o AS $val) {
									?>
									<option value="<?= $val;?>" <?= $_POST['name'] == $val?'selected':false;?>><?= $val;?></option>
									<?
								}
								?>
								</select>
							...
							case 'tel':
								?>
								<input <?= $class;?> id="<?= $name;?>" name="<?= $name;?>" type="tel" value="<?= $_POST[$name];?>" <?= ($args['required']=='true')?'required':false;?>/>
								<?
								break;
							...
							case 'textarea':
								?>
								<textarea <?= $class;?> id="<?= $name;?>" name="<?= $name;?>" <?= ($args['required']=='true')?'required':false;?>><?= $_POST[$name];?></textarea>
								<?
								break;
							
							case 'url':
								?>
								<input <?= $class;?> id="<?= $name;?>" name="<?= $name;?>" type="url" value="<?= $_POST[$name];?>" <?= ($args['required']=='true')?'required':false;?>/>
								<?
								break;
						}
						?>
  ...
  <!-- END WP-Plugin AnyContact http://any-contact.com -->
  <?	
}

function AnyContact_Frontend_SuccessBox($msg) {
	?>
	<span class="pac-success-box"><?= $msg;?></span>
	<?
}

function AnyContact_Frontend_ErrorBox($msg) {
	?>
	<span class="pac-error-box"><?= $msg;?></span>
	<?
}

function AnyContact_Frontend_Error($msg) {
	?>
	<span class="pac-error-msg"><?= $msg;?></span>
	<?
}
